...
describe('UserService', () => {
    ...
    describe('createUser', () => {
        ...
        it('should throw an error if email already exists', async () => {
            ...
            const prismaError = new Prisma.PrismaClientKnownRequestError('Email already exists', {
                code: 'P2002',
                meta: null,
                clientVersion: '',
                batchRequestIdx: 1
    ...
    describe('joinConversation', () => {
        it('should join a conversation', async () => {
            ...
            expect(result.ownerId).toEqual(Number(mockConversation.ownerId));
        });
        ...
        it('should throw an error if conversation not found', async () => {
            ...
            jest.spyOn(prismaService.user, 'findUnique').mockResolvedValue(mockUser);
            jest.spyOn(prismaService.conversation, 'findUnique').mockResolvedValue(null);  // Simulate conversation not found

            await expect(userService.joinConversation(mockUser.id, 123456)).rejects.toThrow('Conversation not found');
        });
